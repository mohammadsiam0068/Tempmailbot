...
function detectOtp(text) {
  if (!text) return null;
  const patterns = [/(?:code|otp|pin|verification code|verify)[^\d]{0,15}(\d{4,8})\b/i, /\b(\d{3}[- ]?\d{3})\b/, /\b(\d{4,8})\b/];
...
function stripHtml(html) {
  return html.replace(/<style[\s\S]*?<\/style>/gi, "").replace(/<script[\s\S]*?<\/script>/gi, "").replace(/<[^>]+>/g, " ").replace(/&nbsp;/g, " ").replace(/&amp;/g, "&").replace(/&lt;/g, "<").replace(/&gt;/g, ">").replace(/ {2,}/g, " ").trim();
}

function mainKeyboard() {
  return { keyboard: [["📧 New Mail", "📥 Inbox"], ["ℹ️ My Email", "🗑 Delete"], ["❓ Help"]], resize_keyboard: true, one_time_keyboard: false };
}

async function sendTelegramMessage(token, chatId, text, options = {}) {
  await fetch(`${TELEGRAM_API_URL}${token}/sendMessage`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ chat_id: chatId, text: text, ...options }) });
}
...
async function handleRequest(request, env) {
  ...
  if (text === "/start") {
    await sendTelegramMessage(env.BOT_TOKEN, chatId, `👋 <b>Welcome, ${escapeHtml(firstName)}!</b>\n\n📬 I'm your <b>Temp Mail Bot</b> — get disposable email addresses instantly!\n\nUse the buttons below:`, { parse_mode: "HTML", reply_markup: mainKeyboard() });
  } else if (text === "/help" || text === "❓ Help") {
    await sendTelegramMessage(env.BOT_TOKEN, chatId, `🤖 <b>Temp Mail Bot — Help</b>\n\n📧 New Mail — Generate a new disposable email\n📥 Inbox — Check received emails\nℹ️ My Email — Show your current email\n🗑 Delete — Delete current email session\n\n<i>Reply with a number to read that email.</i>`, { parse_mode: "HTML", reply_markup: mainKeyboard() });
    ...
    await sendTelegramMessage(env.BOT_TOKEN, chatId, `✅ <b>Your Temp Email is Ready!</b>\n\n📧 <code>${email}</code>\n\n👆 Tap to copy!`, { parse_mode: "HTML", reply_markup: mainKeyboard() });
  } else if (text === "/myemail" || text === "ℹ️ My Email") {
    if (!session) await sendTelegramMessage(env.BOT_TOKEN, chatId, "❌ No active email. Create one first.", { reply_markup: mainKeyboard() });
    else await sendTelegramMessage(env.BOT_TOKEN, chatId, `📧 <b>Your current email:</b>\n<code>${session.email}</code>`, { parse_mode: "HTML", reply_markup: mainKeyboard() });
    ...
    if (!session) await sendTelegramMessage(env.BOT_TOKEN, chatId, "❌ No active email. Create one first.", { reply_markup: mainKeyboard() });
      ...
      try {
        ...
        if (!messages || messages.length === 0) {
          await sendTelegramMessage(env.BOT_TOKEN, chatId, `📭 <b>Inbox is empty.</b>\n\nNo emails received yet for:\n<code>${session.email}</code>`, { parse_mode: "HTML", reply_markup: mainKeyboard() });
        } else {
          session.messages = messages;
          await env.KV_SESSIONS.put(chatId.toString(), JSON.stringify(session));
          const inboxText = `📬 <b>You have ${messages.length} email(s):</b>\n\n` + messages.map((m, i) => `<b>${i + 1}.</b> 📩 From: <code>${escapeHtml(m.from_address)}</code>\n    📌 Subject: ${escapeHtml(m.subject || "(No subject)")}\n    🕐 ${new Date(m.received_at.replace(" ", "T") + "Z").toLocaleString("en-BD", { timeZone: "Asia/Dhaka" })}`).join("\n\n") + `\n\nReply with the number (1, 2, 3...) to read a message`;
    ...
    if (!session) await sendTelegramMessage(env.BOT_TOKEN, chatId, "❌ No active email to delete.", { reply_markup: mainKeyboard() });
    else {
      try { await fetch(`${apiBase}/api/messages?email=${encodeURIComponent(session.email)}`, { method: "DELETE" }); } catch (err) {}
      await env.KV_SESSIONS.delete(chatId.toString());
      await sendTelegramMessage(env.BOT_TOKEN, chatId, `🗑 <b>Email deleted successfully!</b>\n\nUse New Mail to generate a fresh one.`, { parse_mode: "HTML", reply_markup: mainKeyboard() });
    }
  } else if (/^\d+$/.test(text)) {
    const index = parseInt(text) - 1;
    if (!session) await sendTelegramMessage(env.BOT_TOKEN, chatId, "❌ No active session. Create one first.", { reply_markup: mainKeyboard() });
    else if (!session.messages || !session.messages[index]) await sendTelegramMessage(env.BOT_TOKEN, chatId, "❌ Invalid message number.", { reply_markup: mainKeyboard() });
    else {
      ...
      try {
        ...
        if (!full) await sendTelegramMessage(env.BOT_TOKEN, chatId, "⚠️ Could not fetch email.", { reply_markup: mainKeyboard() });
        else {
          let rawBody = "";
          ...
          await sendTelegramMessage(env.BOT_TOKEN, chatId, `📩 <b>Email #${index + 1}</b>\n\n<b>From:</b> <code>${escapeHtml(full.from_address)}</code>\n<b>Subject:</b> ${safeSubject}\n<b>Date:</b> ${new Date(full.received_at.replace(" ", "T") + "Z").toLocaleString("en-BD", { timeZone: "Asia/Dhaka" })}${otpLine}`, { parse_mode: "HTML" });
